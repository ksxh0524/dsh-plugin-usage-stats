 ...
 *  - 日期区间 = 自绘 RangePicker（单触发器 + 预设行 + 月历两段式 + 底部「清除」），不用原生 <input type=date>（宿主深色主题下样式失控）；
 *    全走 --dsw-* 设计令牌。月历格 38px/16px 数字 500 字重（tabular-nums）；选中端点 = brand 蓝实心药丸
 *    （static deepseek-500 + 白字 bluish-00——⚠ 不可用 alias-brand-primary 做填充：那是墨色 token，
 *    浅色主题黑底黑字选中即消失，真机踩实）、区间底 = bg-multi-select、拾取中预览 = hover-accent 半透明
 *    （tentative 与已提交视觉分离）、今天 = alias-link 蓝字。
 *    ⚠ 日格必须经 dayCell(d) 函数参数建独立绑定——循环体内 var + 闭包共享末值曾让整张日历点不动/全点成月末（v3 实锤 bug）。
 *    交互文案从简：预设只剩窗口快捷（今天/近N天），「全部」态由底部「清除」唯一入口达成；不写教程式点选提示文（状态高亮自明）。
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * 报错 = 无边框 12px 错误色文字；文本按钮 = 无边框 label-secondary hover 提亮。
		...
		var CSS = [
			...
			/* 报错文字：theme 真身未定义 label-error（官方 CSS 引用了它但解析为空），用 state-error-primary。 */
			".usg-err{color:var(--dsw-alias-state-error-primary);font-size:12px}",
			...
			".usg-btn[aria-pressed=true]{border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-active)}",
			...
			".usg-pop{position:absolute;top:calc(100% + 4px);left:0;z-index:61;border:0;--dsw-elevation-stroke-color:var(--dsw-alias-border-l2);box-shadow:var(--dsw-elevation-panel);border-radius:8px;background:var(--dsw-specific-menu);padding:10px;font-size:13px;line-height:1.5;min-width:230px}",
			/* 预设 = 官方文本按钮（reset 规范：无边框无底色，secondary hover 提亮） */
			".usg-pre{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:8px}",
			".usg-pre>button{all:unset;cursor:pointer;font-size:14px;line-height:1.5;color:var(--dsw-alias-label-secondary)}",
			...
			".usg-cal-h b{font-size:15px;font-weight:600}",
			".usg-cal-h>button{all:unset;box-sizing:border-box;cursor:pointer;padding:2px 10px;border-radius:6px;font-size:16px;line-height:1.5;color:var(--dsw-alias-label-secondary)}",
			".usg-cal-h>button:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}",
			/* 月历：38px 见方格 + 16px/500 等宽数字（旧版星期头用元素选择器误伤数字格、压成 10px 小字，现头/格分类隔离）。
			 * ⚠ 选中端点 = brand 蓝 static token（deepseek-500 #4176e6）实心 + 白字（bluish-00）——
			 * 绝不能用 alias-brand-primary 当填充：它是「墨色」（浅色主题=#0f1115），invert 也同墨，
			 * 黑底黑字 = 选中数字直接消失（真机踩实）。今天 = alias-link 蓝（双主题都是蓝，不走墨）。
			 * 区间底 = multi-select 填充；拾取中预览 = hover-accent 半透明（tentative 与已提交区分）。 */
			".usg-cal{display:grid;grid-template-columns:repeat(7,38px);gap:2px}",
			".usg-wd{text-align:center;font-size:13px;font-weight:500;color:var(--dsw-alias-label-secondary);line-height:26px}",
			".usg-pv{height:38px;line-height:38px;text-align:center;border-radius:9px;cursor:pointer;font-size:16px;font-weight:500;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary)}",
			".usg-pv:hover{background:var(--dsw-alias-interactive-bg-hover)}",
			".usg-pv[aria-disabled=true]{color:var(--dsw-alias-label-dimmed);font-weight:400;cursor:default}",
			".usg-pv[aria-disabled=true]:hover{background:none}",
			".usg-pv.usg-today{color:var(--dsw-alias-link);font-weight:600}",
			".usg-pv.usg-mid,.usg-pv.usg-mid:hover{background:var(--dsw-alias-bg-multi-select)}",
			".usg-pv.usg-prev,.usg-pv.usg-prev:hover{background:var(--dsw-alias-interactive-bg-hover-accent)}",
			".usg-pv.usg-sel,.usg-pv.usg-sel:hover{background:var(--dsw-static-deepseek-500);color:var(--dsw-static-neutral-bluish-00);font-weight:600}",
			/* 浮层底部只留一个「清除」文本按钮（同样 all:unset——之前未 reset 吃到 UA 默认 13.3px 系统字体显大） */
			...
			".usg-pop-ft>button[aria-disabled=true]{color:var(--dsw-alias-label-dimmed);cursor:default}",
			".usg-pop-ft>button[aria-disabled=true]:hover{background:none;color:var(--dsw-alias-label-dimmed)}",
		...
		function RangePicker(props) {
			...
			/** 单日格。必须走函数参数建独立绑定：曾用循环内 var d 让闭包共享末值，
			 *  结果当月所有格点都被「月末日 > today」吞掉、历史月点谁都是选月末。 */
			function dayCell(d, num) {
				...
				return h(
					...
					{
						key: d,
						className: "usg-pv" + (sel ? " usg-sel" : isMid ? " usg-mid" : isPrev ? " usg-prev" : d === today ? " usg-today" : ""),
			...
			return h(
				Popover,
				{
					...
					trigger: h(
						...
						{
							className: "usg-btn",
							"aria-pressed": open ? "true" : undefined,
				...
				h(
					...
					h(
						"button",
						{
							"aria-disabled": !win && pick === null ? "true" : undefined,
							onClick: function () {
								if (!win && pick === null) return;
								setPick(null);
								setHov(null);
								props.onChange(null);
							},
