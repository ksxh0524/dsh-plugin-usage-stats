...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function modelTable(rows) {
			return h("table", { className: "usg-table" },
				...
				h("tbody", null, rows.map(function (m) {
					return h("tr", { key: m.key },
						...
						h("td", null, fmt(m.requests)), h("td", null, fmt(m.input)),
		...
		function SessionPanel(props) {
			...
			return h("div", { className: "usg-root" },
				...
				data.byModel && data.byModel.length
					? h("div", null, h("div", { className: "usg-title" }, "按模型"), modelTable(data.byModel))
					: null
			);
		}
		...
		function GlobalCard(props) {
			...
			var _y = useState(""), day = _y[0], setDay = _y[1];
			var _d = useState(null), data = _d[0], setData = _d[1];
			var _e = useState(null), err = _e[0], setErr = _e[1];
			var _l = useState(false), loading = _l[0], setLoading = _l[1];
			var _t = useState(0), tick = _t[0], setTick = _t[1];
			...
			var filter = useCallback(function () {
			...
			return h("li", { className: "usg-card" },
				h("header", null,
					h("b", null, "用量统计 · 全局报表"),
				...
				h("div", { className: "usg-bar" },
					h("div", { className: "usg-seg" }, RANGES.map(function (r) {
						return h("button", { key: r[0], "aria-pressed": !day && range === r[0], onClick: function () { setDay(""); setRange(r[0]); } }, r[1]);
					})),
					h("input", {
						className: "usg-date", type: "date", value: day, title: "只看某一天（本地时区）",
				...
				data && data.byModel && data.byModel.length
					? h("div", null, h("div", { className: "usg-title" }, "按模型"), modelTable(data.byModel))
					: null,
				data ? h("div", { className: "usg-muted" },
		...
		function apply(ctx) {
			...
			ctx.effect(function () { return ctx.sidebarRightTabs.register({
				...
				guide: [{ order: 40, title: function () { return "用量统计"; }, description: function () { return "当前会话的 token 用量与命中率（全局报表在设置→插件）" } }]
			...
			ctx.effect(function () { return ctx.slots.inject("settings.plugin.item", function () {
