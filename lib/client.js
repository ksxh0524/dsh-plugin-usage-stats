 ...
 *  另挂 `conversation.composer.dock` 全部 pill（id family-total，order 1）：主会话输入框下方显示
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			/* 全部 dock pill（会话输入框下方，与宿主统计 pill 并排；只在主会话渲染，子代理会话内为 null）。
		...
		function FamilyOptRow() {
			...
			var control =
				UI && UI.Switch
					? h(UI.Switch, { checked: en, onChange: setEn, label: "会话底部全部用量开关" })
					: h("input", {
							type: "checkbox",
							role: "switch",
							"aria-checked": en,
							"aria-label": "会话底部全部用量开关",
							checked: en,
							onChange: function (e) {
			...
			return h(
				...
				h(
					...
					h("span", { className: "usg-optLabel" }, "会话底部“全部”"),
		...
		/** dock 全部 pill：关闭/未知/子代理/零用量 → null；面板走 Anchored（portal + 宿主锚定）。
		...
		function FamilyPill(props) {
			...
			var totalText = "全部 " + fmt(t.total) + " tok";
			var ariaLabel = hit === null ? totalText : totalText + " · 缓存命中 " + hit;
			var models = data.byModel || [];
			var panelChildren = [
				h(
					"div",
					{ className: "usg-famTitle", key: "title" },
					h("span", { className: "usg-famTitleLabel" }, dbIcon(), "全部用量"),
		...
		function apply(ctx) {
			...
			// 会话输入框下方的全部 pill（order 1：宿主自带统计 order 0 之后；只在主会话渲染）。
