/** dsh-plugin-usage-stats 浏览器半（手写 __ModuleLoader__ 工厂，无构建链），v5 交互形态（用户拍板，见 README「交互约定」）：
 ...
 *  总开关关闭时 section 注册即摘掉（ledger 版本号一 bump 宿主左导航 Tab 当场消失；打开即恢复）——
 *  关掉统计还留 Tab 是用户点名要修的（`ctx.slots.inject(): () => void` 回 disposer，动态摘挂有实证）。
 *  会话级视图宿主自带（底部计数条 + 会话统计对话框），本包不重复造——v2 的右侧栏 tab 已删。
 *  另挂 `conversation.composer.dock` 合计 pill（id family-total，order 1）：主会话输入框下方显示
 *  本会话 + 全部子代理合计（未知/子代理/零用量/开关关闭时不渲染，展开面板走 portal + 宿主锚定；
 *  自动刷新：子代理开/关、本会话落步、3s 轮询三路触发，出现与涨数都不用手动刷新页）。
 *  另挂 `plugins.item` 配置卡（id usage-stats，order 110）：两开关（Token 用量统计总开关 /
 *  对话框底下显示），**拨动即写**（setConfig 直落宿主 settings.yaml 热推送，无草稿/保存/丢弃），
 *  点进详情内容直接可见（默认展开、不折叠）。页内分节同样常开不折叠（静态组标题）。
 *  ⚠ 以上三处偏离宿主默认规范（card-form 草稿+保存唯一写点、卡默认折叠、页内组折叠头）——
 *  宿主形是防误写/防占位，本包两开关都是可逆布尔、内容只有两行，用户明确要少点一次。
 *
 ...
 *  - **页内分节（本包用户拍板常开）**：明细表各成一个 `<section>` 组，组头是静态 `<h3>`
 *    + 计数副行——无折叠头/chevron（偏离宿主 inventory 折叠默认，见 README「交互约定」）。
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CONTRIBUTION = {
			...
			descriptors: [
				descriptor("overview", "filter", { file: "src/cordis.ts", line: 87, column: 9 }),
				descriptor("familyTotal", "filter", { file: "src/cordis.ts", line: 98, column: 9 }),
				descriptor("getConfig", "_hint", { file: "src/cordis.ts", line: 108, column: 9 }),
				descriptor("setConfig", "patch", { file: "src/cordis.ts", line: 115, column: 9 }),
			],
		};
		...
		var CSS = [
			...
			// 分节组（常开不折叠：用户拍板，点进页内容直接可见；组头是静态 <h3> + 计数副行，无折叠按钮）
			".usg-group+.usg-group{border-top:.5px solid var(--dsw-alias-border-l2);padding-top:14px}",
			".usg-groupTitle{margin:0;font-size:12px;font-weight:600;line-height:1.5;letter-spacing:.06em;text-transform:uppercase;color:var(--dsw-alias-label-tertiary)}",
			...
			/* 合计 dock pill（会话输入框下方，与宿主统计 pill 并排；只在主会话渲染，子代理会话内为 null）。
			 *  pill 与面板照抄宿主 StatsPills / stat-dialog 同位样式（同字号/同行高/同 hover 胶囊/同面板；
			 *  唯一偏离：分隔点颜色不用 --dsw-alias-separator-primary——该 token 在已装宿主主题里不存在，
			 *  用 label-dimmed 代替，token 审计不过会红）。
			 *  面板走 Anchored（portal + 宿主锚定/关闭 hook，与月历同一骨架），in-flow 旧版已删。 */
			".usg-fam{display:inline-flex;min-width:0}",
			".usg-famPill{display:inline-flex;align-items:center;gap:6px;box-sizing:border-box;max-width:100%;padding:1px 8px;border:0;border-radius:24px;background:transparent;color:var(--dsw-alias-label-tertiary);font-size:13px;line-height:1.55;font-variant-numeric:tabular-nums;white-space:nowrap;cursor:pointer}",
			".usg-famPill:hover,.usg-famPill[aria-expanded=true]{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-secondary)}",
			".usg-famPill:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}",
			".usg-famPill svg{width:14px;height:14px;flex:none}",
			".usg-famLabel{min-width:0;overflow:hidden;text-overflow:ellipsis}",
			".usg-famPanel{position:fixed;z-index:1100;box-sizing:border-box;width:max-content;min-width:min(300px,calc(100vw - 24px));max-width:min(440px,calc(100vw - 24px));padding:16px;border:0;border-radius:12px;background:var(--dsw-specific-menu);--dsw-elevation-stroke-color:var(--dsw-alias-border-l1);box-shadow:var(--dsw-elevation-prominent);font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary);cursor:default}",
			".usg-famTitle{display:flex;justify-content:space-between;gap:16px;margin-bottom:8px;color:var(--dsw-alias-label-primary);font-weight:500}",
			".usg-famTitleLabel{display:inline-flex;align-items:center;gap:6px;min-width:0}",
			".usg-famTitleLabel svg{width:14px;height:14px;flex:none}",
			".usg-famTitleValue{font-variant-numeric:tabular-nums}",
			".usg-famRule{margin-bottom:10px;border-top:.5px solid var(--dsw-alias-border-l2)}",
			".usg-famDetails{display:grid;grid-template-columns:minmax(76px,auto) minmax(0,1fr);gap:6px 16px;margin:0;color:var(--dsw-alias-label-tertiary)}",
			".usg-famDetails dt,.usg-famDetails dd{min-width:0;margin:0}",
			".usg-famDetails dd{color:var(--dsw-alias-label-secondary);font-variant-numeric:tabular-nums;text-align:right}",
			".usg-famRoute{overflow-wrap:anywhere}",
			/* 插件卡（插件管理页 plugins.item：常开 li 卡壳 + 拨动即写，无折叠/草稿（用户拍板偏离
			 *  宿主 card-form 默认，见 README「交互约定」）。值照抄宿主 PluginConfigForm/fields。
			 * 结构见索引仓 docs/settings-cards.md §1.1/§1.2。无 transition（动效守卫零负担）。 */
			".usg-card{list-style:none;border:.5px solid var(--dsw-alias-border-l4);border-radius:16px;background:var(--dsw-alias-bg-layer-3)}",
			".usg-card:hover{border-color:var(--dsw-alias-label-dimmed)}",
			".usg-card:focus-within{border-color:var(--dsw-alias-label-dimmed)}",
			".usg-cardOpen{background:var(--dsw-alias-bg-layer-2);border-color:var(--dsw-alias-label-dimmed)}",
			".usg-cardHead{appearance:none;width:100%;display:flex;align-items:center;gap:12px;padding:14px 16px;border:0;border-radius:12px;background:none;font:inherit;color:inherit;text-align:left;cursor:pointer}",
			".usg-cardHead:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:-2px}",
			".usg-cardStatic{cursor:default}",
			".usg-cardHeadText{flex:1;min-width:0;display:flex;flex-direction:column;gap:4px}",
			".usg-cardName{font-size:15px;font-weight:600;line-height:1.4;color:var(--dsw-alias-label-primary)}",
			".usg-cardDesc{font-size:13px;line-height:1.5;color:var(--dsw-alias-label-tertiary)}",
			".usg-cardBody{border-top:.5px solid var(--dsw-alias-border-l2);margin:0 16px;padding-bottom:8px}",
			".usg-cardReadonly{margin:12px 0 0;font-size:12px;line-height:1.5;color:var(--dsw-alias-label-tertiary)}",
			".usg-field{padding:12px 0}",
			".usg-field+.usg-field{border-top:.5px solid var(--dsw-alias-border-l2)}",
			".usg-fieldRow{display:flex;align-items:center;gap:10px}",
			".usg-fieldMain{flex:1;min-width:0;display:flex;flex-direction:column;gap:2px}",
			".usg-fieldLabel{font-size:13px;font-weight:500;line-height:1.5;color:var(--dsw-alias-label-primary)}",
			".usg-fieldHint{margin:0;font-size:12px;line-height:1.5;color:var(--dsw-alias-label-tertiary)}",
			".usg-field input[type=checkbox]{width:16px;height:16px;flex:none;accent-color:var(--dsw-alias-label-secondary)}",
			".usg-cardFoot{display:flex;justify-content:flex-end;align-items:center;gap:8px;padding:12px 0 4px}",
			".usg-cardErr{margin:0 auto 0 0;font-size:12px;line-height:1.5;color:var(--dsw-alias-state-error-primary)}",
			".usg-retryBtn{appearance:none;border:.5px solid var(--dsw-alias-border-l2);border-radius:8px;padding:5px 14px;font:inherit;font-size:13px;line-height:1.5;background:none;color:var(--dsw-alias-label-secondary);cursor:pointer}",
			".usg-retryBtn:hover:not(:disabled){color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-label-dimmed)}",
			".usg-retryBtn:disabled{opacity:.4;cursor:default}",
			".usg-retryBtn:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}",
			// 动效自护（§4.3；宿主 ui-theme 无全局兜底；本包已无折叠 chevron，无需 transition 豁免）
		].join("\n");
		var cssDone = false;
		function ensureCss() {
		...
		/** 精确数（千分位）：紧凑显示的 hover 精确值，分项与总数可逐项核对（B 档 2 位小数是 10M 粒度，各自舍入后视觉和≠总数是正常的）。 */
		function fmtFull(n) {
			if (typeof n !== "number" || !isFinite(n)) return "—";
		...
		/** 宿主 pill 缓存命中段同形：整数档、满命中给 100、差一点满自动加小数保真（照抄上游 token-format.ts，decimalPlaces=0 那条路）。
		 *  入参是精确桶（分子=缓存读，分母=未缓存输入+缓存读+缓存写，与宿主 billedInputTokens 同口径）；分母为 0 回 null（整段不渲染）。 */
		function roundedHitUnits(read, denom) {
			var scale = 100,
				doubled = 200;
			var q = Math.floor(denom / doubled),
				r = denom % doubled;
			var lo = 0,
				hi = 100;
			while (lo < hi) {
				var c = Math.floor((lo + hi + 1) / 2),
					f = c * 2 - 1;
				if (read >= f * q + Math.ceil((f * r) / doubled)) lo = c;
				else hi = c - 1;
			}
			return lo;
		}
		function fmtHit(read, denom) {
			if (!(denom > 0)) return null;
			if (denom - read === 0) return "100";
			var u = roundedHitUnits(read, denom);
			if (u < 100) return String(u);
			var places = 1,
				gap = (denom - read) * 200,
				tens = Math.floor(denom / 10);
			while (gap <= tens) {
				gap *= 10;
				places += 1;
			}
			...
			for (var l = 1; l < 5; l++) {
				var f = l * 2 + 1;
				if (gap <= f * tens + Math.floor((f * ones) / 10)) {
					loss = l;
					break;
				}
			}
			var s = "99.";
			for (var i = 1; i < places; i++) s += "9";
			return s + (10 - loss);
		}
		/** 宿主对话框精确数同形：具体数千分位 + " tok" 由调用方拼。 */
		function fmtExact(n) {
			if (typeof n !== "number" || !isFinite(n)) return "—";
			return String(Math.round(n)).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
		}
		/** 宿主用量 pill 同款数据库图标（primitives 缺席回 null，不手搓替身）。 */
		function dbIcon() {
			if (UI && UI.IconDatabaseOutline16) return h(UI.IconDatabaseOutline16, { "aria-hidden": "true" });
			return null;
		}
		function p2(n) {
			return (n < 10 ? "0" : "") + n;
		...
		/* ---------- 分节组（常开静态节：无折叠头，内容永远在 DOM——用户拍板，点进页直接可见） ---------- */
		function Group(props) {
			return h(
				"section",
				{ className: "usg-group", "aria-label": props.title },
				h("h3", { className: "usg-groupTitle" }, props.title),
		...
		function Anchored(props) {
			...
			var hostPos =
				UI && UI.useAnchoredPosition
					? UI.useAnchoredPosition({ open: props.open, anchorRef: rootRef, panelRef: panelRef, side: props.side || "bottom", gap: props.gap || 4, margin: 12 })
					: null;
			...
			useEffect(
				function () {
					...
					var ph = panelRef.current ? panelRef.current.offsetHeight : 0;
					var top = props.side === "top" ? r.top - 8 - ph : r.bottom + 4;
					setFallbackPos({ position: "fixed", left: r.left, top: Math.max(12, top) });
				},
				[props.open],
			);
			var panel = props.open
				? h(
						...
						{
							ref: panelRef,
							className: (props.panelClass || "usg-pop") + (hostPos || fallbackPos ? "" : " usg-popInline"),
							// 宿主 useStatDialog 同形：首帧隐藏占位供测量，定位到了再显示（不闪一下错位）。
							style: hostPos || fallbackPos || { visibility: "hidden", left: 0, top: 0 },
		...
		function kpi(label, value, sub, title) {
			return h(
				...
				{ className: "usg-kpi" },
				h("dt", null, label),
				h("dd", title ? { title: title } : null, value),
				sub ? h("dd", { className: "usg-kpiSub" }, sub) : null,
			);
		}
		...
		function DimSelect(props) {
			...
			// 兜底（primitives 缺席）：原生 select，语义与键盘由浏览器保证（label 包裹 + aria-label 双关联，卡门要点名）。
			return h(
				"label",
			...
			return h(
				...
				data && !err
					? h(
							"dl",
							{ className: "usg-kpis" },
							kpi("会话数", fmt(data.sessionCount), "扫描 " + fmt(data.scannedFiles) + " 个文件", fmtFull(data.sessionCount)),
							kpi("总 tokens（含缓存）", fmt(t.total), fmt(t.requests || 0) + " 次请求", fmtFull(t.total)),
							kpi("未缓存输入", fmt(t.input), undefined, fmtFull(t.input)),
							kpi("输出", fmt(t.output), undefined, fmtFull(t.output)),
							kpi("缓存读", fmt(t.cacheRead), "命中率 " + fmtPct(data.hitRate), fmtFull(t.cacheRead)),
							kpi("缓存写", fmt(t.cacheWrite), undefined, fmtFull(t.cacheWrite)),
						)
					: null,
				data
					? h(
								...
								: table(
										...
										rows.map(function (m) {
											return {
												...
												cells: [
													{ v: m.key, t: m.key },
													{ v: fmt(m.requests), t: fmtFull(m.requests) },
													{ v: fmt(m.input), t: fmtFull(m.input) },
													{ v: fmt(m.output), t: fmtFull(m.output) },
													{ v: fmt(m.cacheRead), t: fmtFull(m.cacheRead) },
													{ v: fmtPct(m.hitRate) },
												],
											};
										}),
		...
		/* ---------- 插件卡：plugins.item 常开卡（两开关，拨动即写，无草稿/保存/丢弃） ---------- */
		/** 卡标题/简介/排序（官方四卡 order 10/20/30/40，自研卡排之后；id 必须 === settings 命名空间名）。 */
		var CARD_TITLE = "Token 用量";
		var CARD_DESC = "用量统计总开关与输入框合计 pill 的开关；拨动即保存。";
		var CARD_ORDER = 110;
		/** section 页显隐控制器（apply 内装真实实现：总开关写成功后直调，另有 document-updated 订阅兜底异地写；
		 *  装配前是空函数——apply 的 effect 先于任何组件渲染执行，正常路径不会打空）。 */
		var sectionCtl = {
			setVisible: function () {},
		};
		/** 开关行（宿主 Switch 常路，缺席退原生 checkbox；label 真关联，读屏可达）。
		 *  id 供 label htmlFor（门要真关联；Switch 自带 label 名，checkbox 走 htmlFor + aria-label 双保险）。 */
		function boolRow(id, label, hint, value, onFlip, disabled) {
			var control =
				UI && UI.Switch
					? h(UI.Switch, { checked: !!value, onChange: onFlip, label: label, disabled: !!disabled })
					: h("input", {
							type: "checkbox",
							role: "switch",
							id: id,
			...
			return h(
				"div",
				{ className: "usg-field", key: id },
				h(
					"div",
					{ className: "usg-fieldRow" },
					h(
						"div",
						{ className: "usg-fieldMain" },
						h("label", { className: "usg-fieldLabel", htmlFor: id }, label),
						h("p", { className: "usg-fieldHint" }, hint),
					),
					control,
				),
			);
		}
		/** 插件卡本体（view 双态在 UsageEntry 分流；本组件只负责 page 完整表单）。
		 *  用户拍板形态：内容常开（无折叠头）、拨动即写（无草稿/保存/丢弃）。
		 *  写状态机：confirmed = 服务端已确认值；pending = 在途写（渲染优先它，失败清掉即自动回滚）；
		 *  seq 代际只认末次回执（连拨竞态不翻旧账）。失败留行内错误 + 重试，不吞值。 */
		function UsageCard(props) {
			...
			var load = useCallback(
				async function () {
					...
					try {
						await mounted;
						var r = await ctx.get("remote.usageStats").getConfig({});
						if (!r.ok) throw new Error(r.error.message);
						if (alive.current === my) setLoaded(r.value);
					} catch (e) {
						if (alive.current === my) setErr(humanizeError(String((e && e.message) || e)));
					}
				},
			...
			var confirmed = loaded ? { familyEnabled: !!loaded.config.familyEnabled, dockVisible: !!loaded.config.dockVisible } : null;
			var shown = pending || confirmed;
			var readonly = !!(loaded && loaded.writable === false);
			var saving = !!pending;
			// 拨动即写：整对象 setConfig 直落盘（settings.yaml 热推送）；成功回执刷新确认值并直调 Tab 显隐。
			var write = function (obj) {
				...
				(async function () {
					try {
						await mounted;
						var r = await ctx.get("remote.usageStats").setConfig(obj);
						if (!r.ok) throw new Error(r.error.message);
						if (seq.current !== my) return;
						setLoaded({ config: r.value, writable: true });
						setPending(null);
						sectionCtl.setVisible(!!r.value.familyEnabled);
					} catch (e) {
						if (seq.current !== my) return;
						setPending(null);
						setFailed(obj);
			...
			var flip = function (key) {
				return function (next) {
					if (!shown || readonly || pending) return;
					var obj = { familyEnabled: !!shown.familyEnabled, dockVisible: !!shown.dockVisible };
					obj[key] = !!next;
					write(obj);
				};
			};
			...
			return h(
				"li",
				{ className: "usg-card usg-cardOpen" },
				h(
					"div",
					{ className: "usg-cardHead usg-cardStatic" },
					h(
						"span",
						{ className: "usg-cardHeadText" },
						h("span", { className: "usg-cardName" }, CARD_TITLE),
						h("span", { className: "usg-cardDesc" }, CARD_DESC),
					),
				),
				h(
					"div",
					{ className: "usg-cardBody" },
					readonly ? h("p", { className: "usg-cardReadonly" }, "只读：当前设置为只读文档，开关已禁用。") : null,
					err && !loaded
						? h(
								"div",
								null,
								h("p", { className: "usg-failure", role: "alert" }, "读取失败：" + err),
								h("button", { type: "button", className: "usg-retry", onClick: load }, "重试"),
							)
						: null,
					loaded
						? h(
								...
								boolRow(
									"usg-familyEnabled",
									"Token 用量统计",
									"总开关：关闭后设置里的「Token 用量」页与输入框合计 pill 一起隐藏；插件卡常在，随时可开回。",
									shown && shown.familyEnabled,
									flip("familyEnabled"),
									readonly || saving,
								),
								boolRow(
									"usg-dockVisible",
									"对话框底下显示",
									"主会话输入框下方是否显示合计 pill（只在开过子代理的主会话出现；出现与涨数自动刷新，不用手动刷新页面）。",
									shown && shown.dockVisible,
									flip("dockVisible"),
									readonly || saving,
								),
								saving ? h("p", { className: "usg-status", role: "status" }, "保存中…") : null,
								err && !saving
									? h(
											"div",
											{ className: "usg-cardFoot" },
											h("p", { className: "usg-cardErr", role: "status" }, "保存失败：" + err),
											h("button", { type: "button", className: "usg-retryBtn", onClick: retry }, "重试"),
										)
									: null,
							)
						: null,
		...
		/** plugins.item 入口（view 双态：summary 一句话简介 / page 完整表单）。 */
		function UsageEntry(props) {
			if (props && props.view === "summary") return CARD_DESC;
			return h(UsageCard, { ctx: props.ctx, mounted: props.mounted });
		}
		...
		/** 全部面板的桶行（宿主 UsagePill 对话框同行同序：命中/未缓存输入/缓存读取/缓存写入/输出；写缓存为 0 不占行）。 */
		function famBucketCells(t, hit) {
			var cells = [];
			if (hit !== null) {
				cells.push(h("dt", { key: "h" }, "缓存命中"));
				cells.push(h("dd", { key: "hv" }, hit + "%"));
			}
			cells.push(h("dt", { key: "i" }, "未缓存输入"));
			cells.push(h("dd", { key: "iv" }, fmtExact(t.input) + " tok"));
			cells.push(h("dt", { key: "r" }, "缓存读取"));
			cells.push(h("dd", { key: "rv" }, fmtExact(t.cacheRead) + " tok"));
			if (t.cacheWrite !== 0) {
				cells.push(h("dt", { key: "w" }, "缓存写入"));
				cells.push(h("dd", { key: "wv" }, fmtExact(t.cacheWrite) + " tok"));
			}
			cells.push(h("dt", { key: "o" }, "输出"));
			cells.push(h("dd", { key: "ov" }, fmtExact(t.output) + " tok"));
			return cells;
		}
		/** 存活后代计数（宿主 useSessions 快照 → 数字）：以 sessionId 为祖先的存活会话数（显示与否仍由
		 *  服务端 sessionCount 判定，这里只当刷新触发器）。返回数字（引用稳定）：只有子代理开/关才变，
		 *  无关会话的标题/选中变化不引起重渲染。环形血缘按 seen 截断，不死循环。 */
		function famLiveCount(state, sessionId) {
			if (!state || !state.byId || !sessionId) return 0;
			var byId = state.byId,
				n = 0,
				id,
				cur,
				seen;
			for (id in byId) {
				if (id === sessionId || !byId[id]) continue;
				cur = byId[id];
				seen = {};
				while (cur && cur.parentId) {
					if (cur.parentId === sessionId) {
						n++;
						break;
					}
		...
		/** 合计值签名：渲染依赖的全部字段拍扁——轮询/投影抖动回来的等值结果不 setState，不空转重渲染。 */
		function famSig(v) {
			if (!v) return "";
			var t = v.totals || {};
			var rows = (v.byModel || [])
				.map(function (m) {
					return m.key + "=" + m.total;
				})
				.join(",");
			return [v.enabled, v.dockVisible, v.known, v.isSubagent, v.sessionCount, t.total, t.input, t.output, t.cacheRead, t.cacheWrite, rows].join("|");
		}
		/** dock 合计 pill 调度器：有宿主标准席位（useSessions/useProjection，dock 槽 scope=session 恒有，
		 *  上游 slots.md 言明与注册包无关）走自动刷新活路；缺席（旧宿主/单测直挂）退一次性旧路。 */
		function FamilyPill(props) {
			var sessionId = normSessionId(props && props.familySessionId);
			if (props && typeof props.useSessions === "function" && typeof props.useProjection === "function")
				return h(FamilyPillLive, { sessionId: sessionId, familyTotal: props.familyTotal, useSessions: props.useSessions, useProjection: props.useProjection });
			return h(FamilyPillOnce, { sessionId: sessionId, familyTotal: props.familyTotal });
		}
		/** 一次性旧路（无标准席位时的退路）：挂载/session 切换/开面板时拉一次，与旧版行为一致。 */
		function FamilyPillOnce(props) {
			...
			return h(FamPillView, {
				data: data,
				onRefresh: load,
			});
		}
		/** 自动刷新活路：首拉 + 三路触发（子代理开/关、本会话 token 涨、3s 轮询兜底子代理侧涨数），
		 *  pill 自动出现、数字自动跟上，不再靠手动刷新页。在途请求未回时跳过（不叠请求），
		 *  页签 hidden 时跳过（不打扰后台）。服务端是增量扫描，未变文件只做 stat，空转便宜。 */
		function FamilyPillLive(props) {
			var sessionId = props.sessionId;
			var _d = useState(null),
				data = _d[0],
				setData = _d[1];
			...
			var load = useCallback(
				async function () {
					if (!sessionId || typeof props.familyTotal !== "function" || busy.current) return;
					busy.current = true;
					var my = ++alive.current;
					try {
						var r = await props.familyTotal({ sessionId: sessionId });
						...
						/* 输入区徽标：失败即保持现状，不打扰输入（全量错误面在设置页）。 */
					} finally {
						if (alive.current === my) busy.current = false;
					}
				},
				[props.familyTotal, sessionId],
			);
			// ① 挂载/session 切换：作废旧在途、清签名、首拉。
			useEffect(
				function () {
					alive.current++;
					busy.current = false;
			...
			// ② 子代理开/关：宿主会话表里的后代计数一变就重拉，pill 自动出现/消失。
			var liveCount = props.useSessions(function (s) {
				return famLiveCount(s, sessionId);
			});
			useEffect(
				function () {
					if (sessionId) load();
				},
				[sessionId, load, liveCount],
			);
			// ③ 本会话 token 涨：主会话每落定一步就重拉，与宿主自带 pill 同步跟上。
			var ownUsage = props.useProjection("tokenUsage");
			useEffect(
				function () {
					if (sessionId) load();
				},
				[sessionId, load, ownUsage],
			);
			// ④ 3s 轮询兜底：子代理侧涨的 token 只落它自己会话文件，本会话收不到投影推送，只能轮询。
			useEffect(
				function () {
					if (!sessionId || typeof setInterval === "undefined") return;
		...
		/** 合计 pill 视图（两路数据共用）：关闭/未知/子代理/零用量 → null；面板走 Anchored（portal + 宿主锚定）。
		 *  单模型不展模型区；多模型才加「按模型」一节（行 = 模型键 → 紧凑总数）。 */
		function FamPillView(props) {
			var data = props.data;
			var _o = useState(false),
				open = _o[0],
				setOpen = _o[1];
			// 显示条件：服务端两开关全开 + 开过子代理的主会话（sessionCount>1），一看两数不一样就知道哪个是哪个；
			// 关闭/未知/子代理/零用量 → null。可见文案只有数字（宿主 pill 同形，不挂名前缀与命中段）。
			if (
				!data ||
				data.enabled !== true ||
				data.dockVisible !== true ||
				!data.known ||
				data.isSubagent ||
				!data.totals ||
				!(data.totals.total > 0) ||
				...
				return null;
			var t = data.totals;
			var hit = fmtHit(t.cacheRead, t.input + t.cacheRead + t.cacheWrite);
			var totalText = fmt(t.total) + " tok";
			// 可见文案与宿主 pill 同形（只显示数字，不挂名前缀）；合计身份只进读屏 aria-label。命中段同宿主：整数，无输入整段不渲染。
			var ariaLabel = "合计 " + (hit === null ? totalText : totalText + " · 缓存命中 " + hit + "%");
			var models = data.byModel || [];
			var panelChildren = [
				h(
					"div",
					{ className: "usg-famTitle", key: "title" },
					h("span", { className: "usg-famTitleLabel" }, dbIcon(), "合计用量"),
					h("span", { className: "usg-famTitleValue" }, fmtExact(t.total) + " tok"),
				),
				h("div", { className: "usg-famRule", key: "rule", "aria-hidden": "true" }),
				h("dl", { className: "usg-famDetails", key: "buckets" }, famBucketCells(t, hit)),
			];
			if (models.length > 1) {
				var modelCells = [];
				models.forEach(function (m, i) {
					modelCells.push(h("dt", { key: "m" + i, className: "usg-famRoute" }, m.key));
					modelCells.push(h("dd", { key: "mv" + i, title: fmtFull(m.total) + " tok" }, fmt(m.total) + " tok"));
				});
				panelChildren.push(h("div", { className: "usg-famRule", key: "mrule", "aria-hidden": "true", style: { marginTop: "10px" } }));
				panelChildren.push(
					h(
						"div",
						{ className: "usg-famTitle", key: "mtitle" },
						h("span", { className: "usg-famTitleLabel" }, "按模型"),
						h("span", { className: "usg-famTitleValue" }, models.length + " 个"),
					),
				);
				panelChildren.push(h("dl", { className: "usg-famDetails", key: "models" }, modelCells));
			}
			return h(Anchored, {
				open: open,
				onClose: function () {
				...
				label: "合计用量",
				panelClass: "usg-famPanel",
				// 宿主 useStatDialog 同值：面板固定在触发器上方 8px，视口内钳制（底部 dock 朝下开会盖住输入区）。
				side: "top",
				gap: 8,
				trigger: h(
					"button",
					{
						...
						title: fmtFull(t.total) + " tok",
						onClick: function () {
							var next = !open;
							setOpen(next);
							if (next && typeof props.onRefresh === "function") props.onRefresh();
						},
					},
					dbIcon(),
					h("span", { className: "usg-famLabel" }, totalText),
				),
		...
		function apply(ctx) {
			...
			// 总开关连 Tab（用户拍板）：familyEnabled=false 时摘掉本注册（ledger bump → 左导航 Tab 当场消失），
			// 打开即恢复。inject 回 disposer（网关签名 inject(): () => void），动态摘挂有实证。
			ctx.effect(function () {
				var disposeSection = null;
				function registerSection() {
					if (disposeSection) return;
					var r = ctx.slots.inject("settings.section", function () {
					...
					if (typeof r === "function") disposeSection = r;
				}
				function setVisible(on) {
					if (on) registerSection();
					else if (disposeSection) {
						disposeSection();
						disposeSection = null;
					}
				}
				sectionCtl.setVisible = setVisible;
				registerSection(); // 默认开：先挂，远端回执关再摘；配置读不到 fail-open（Tab 留着，不动现状）。
				function reconcile() {
					(async function () {
						try {
							await mounted;
							var remote = ctx.get("remote.usageStats");
							if (!remote || typeof remote.getConfig !== "function") return;
							var r = await remote.getConfig({});
							if (!r.ok) return;
							setVisible(!!r.value.config.familyEnabled);
						} catch (e) {
							/* 读不到配置 = 沿用现状，不动 Tab。 */
						}
					})();
				...
				var offDoc = null;
				try {
					if (ctx.remote && typeof ctx.remote.$on === "function") offDoc = ctx.remote.$on("settings/document-updated", reconcile);
				} catch (e) {
					offDoc = null;
				}
				reconcile();
			...
			// 插件管理页的本插件配置卡（plugins.item list 槽：id 必须 === settings 命名空间名，
			// 即服务端半 installSection("usage-stats") 的 served namespace——两端靠它配对，缺一隐身）。
			ctx.effect(function () {
				return ctx.slots.inject("plugins.item", function () {
					return ctx.slots.register({ name: "plugins.item", id: SETTINGS_NS, order: CARD_ORDER, label: CARD_TITLE }, function (slotProps) {
						return h(UsageEntry, { ctx: ctx, mounted: mounted, view: slotProps && slotProps.view });
					});
				});
			}, "usage-stats: plugin card");
			// 会话输入框下方的合计 pill（order 1：宿主自带统计 order 0 之后；只在主会话渲染）。
