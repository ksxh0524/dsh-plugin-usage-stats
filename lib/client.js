 ...
 *  另挂 `plugins.item` 配置卡（id usage-stats，order 110）：合计两开关（familyEnabled 总开关 /
 *  dockVisible 对话框底下显示），草稿 + 保存唯一写点，落宿主 settings.yaml 热推送。
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			/* 插件卡（插件管理页 plugins.item：折叠 li 卡壳 + 暂存草稿 + 保存唯一写点；值照抄宿主 PluginConfigForm/fields。
			 * 结构见索引仓 docs/settings-cards.md §1.1/§1.2。无 transition（动效守卫零负担，chevron 翻转即时）。 */
			...
			".usg-cardDirty{font-size:11px;line-height:1.4;color:var(--dsw-alias-state-error-primary)}",
			".usg-cardChev{flex:none;color:var(--dsw-alias-label-tertiary)}",
			".usg-cardChevOpen{transform:rotate(180deg)}",
			...
			".usg-cardFoot{display:flex;justify-content:flex-end;align-items:center;gap:8px;border-top:.5px solid var(--dsw-alias-border-l2);padding:12px 0 4px}",
			".usg-cardErr{margin:0 auto 0 0;font-size:12px;line-height:1.5;color:var(--dsw-alias-state-error-primary)}",
			".usg-discard,.usg-save{appearance:none;border:1px solid transparent;border-radius:8px;padding:5px 14px;font:inherit;font-size:13px;line-height:1.5;cursor:pointer}",
			".usg-discard{border-color:var(--dsw-alias-border-l2);background:none;color:var(--dsw-alias-label-secondary)}",
			".usg-discard:hover:not(:disabled){color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-label-dimmed)}",
			".usg-save{background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3)}",
			".usg-save:hover:not(:disabled){filter:brightness(1.12)}",
			".usg-discard:disabled,.usg-save:disabled{opacity:.4;cursor:default}",
			".usg-discard:focus-visible,.usg-save:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}",
			...
			return h(
				...
				data && !err
					? h(
							...
							kpi("输入（未缓存）", fmt(t.input), undefined, fmtFull(t.input)),
		...
		function dirtyTag() {
			if (UI && UI.Tag) return h(UI.Tag, { tone: "neutral", className: "usg-pending" }, "未保存");
			return h("span", { className: "usg-cardDirty" }, "未保存");
		}
		function chevIcon(open) {
			var cls = "usg-cardChev" + (open ? " usg-cardChevOpen" : "");
			if (UI && UI.IconChevronDownOutline14) return h(UI.IconChevronDownOutline14, { className: cls, "aria-hidden": "true" });
		...
		function UsageCard(props) {
			...
			var load = useCallback(
				async function () {
					...
					try {
						...
						if (alive.current === my) {
							setLoaded(r.value);
							setDraft({ familyEnabled: !!r.value.config.familyEnabled, dockVisible: !!r.value.config.dockVisible });
						}
					} catch (e) {
						if (alive.current === my) setErr(humanizeError(String((e && e.message) || e)));
					}
			...
			var dirty = !!(loaded && draft && (draft.familyEnabled !== !!loaded.config.familyEnabled || draft.dockVisible !== !!loaded.config.dockVisible));
			var readonly = !!(loaded && loaded.writable === false);
			var flip = function (key) {
				return function (next) {
					setDraft(function (f) {
						var n = { familyEnabled: !!f.familyEnabled, dockVisible: !!f.dockVisible };
						n[key] = !!next;
						return n;
					});
				};
			...
			var discard = function () {
				if (!loaded) return;
				setDraft({ familyEnabled: !!loaded.config.familyEnabled, dockVisible: !!loaded.config.dockVisible });
				setErr(null);
			};
			// 保存是唯一写点： switched 草稿经 setConfig 一次落盘（带 revision 的文档变更，不由输入过程触发）。
			var save = async function () {
				...
				try {
					await mounted;
					var r = await ctx.get("remote.usageStats").setConfig({ familyEnabled: !!draft.familyEnabled, dockVisible: !!draft.dockVisible });
					if (!r.ok) throw new Error(r.error.message);
			...
			return h(
				"li",
				{ className: "usg-card" + (open ? " usg-cardOpen" : "") },
				h(
					"button",
					{
						type: "button",
						className: "usg-cardHead",
						"aria-expanded": open,
							...
							loaded
								? h(
										...
										boolRow(
											"usg-familyEnabled",
											"合计统计",
											"关闭后输入框合计 pill 与合计数据一起停用（全局用量页不受影响）。",
											draft && draft.familyEnabled,
											flip("familyEnabled"),
											readonly || saving,
										),
										boolRow(
										...
										h(
											...
											err ? h("p", { className: "usg-cardErr", role: "status" }, "保存失败：" + err) : null,
											h("button", { type: "button", className: "usg-discard", disabled: !dirty || saving, onClick: discard }, "丢弃"),
											h(
												"button",
												{ type: "button", className: "usg-save", disabled: !dirty || saving || readonly, onClick: save },
												saving ? "保存中…" : "保存",
											),
										),
									)
