...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* ---------- 样式（materialize 时注入一次；全部 --dsw-* 令牌，跟随宿主主题） ---------- */
		var CSS = [
			...
			".usg-btn{all:unset;box-sizing:border-box;cursor:pointer;padding:4px 12px;border:1px solid var(--dsw-alias-separator);border-radius:6px;font-size:12px;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-fill-quinary)}",
			".usg-btn:hover{color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-label-tertiary)}",
			".usg-btn[aria-pressed=true],.usg-pop .usg-pv[aria-selected=true]{background:var(--dsw-alias-accent-fill,var(--dsw-alias-fill-quaternary));color:var(--dsw-alias-label-primary)}",
			".usg-title{font-size:12px;font-weight:600;color:var(--dsw-alias-label-secondary);margin-bottom:4px}",
			/* 浮层（RangePicker / DimSelect 共用）：透明 mask 兜外点关闭，panel 挂触发器下方 */
			".usg-anchor{position:relative}",
			".usg-mask{position:fixed;inset:0;z-index:60}",
			".usg-pop{position:absolute;top:calc(100% + 4px);left:0;z-index:61;border:1px solid var(--dsw-alias-separator);border-radius:10px;background:var(--dsw-alias-elevated-background, var(--dsw-alias-background, rgba(30,30,30,.98)));box-shadow:0 8px 28px rgba(0,0,0,.35);padding:10px;font-size:12px;min-width:230px}",
			".usg-pre{display:flex;gap:4px;flex-wrap:wrap;margin-bottom:8px}",
			".usg-pre>button{all:unset;cursor:pointer;padding:2px 8px;border:1px solid var(--dsw-alias-separator);border-radius:999px;color:var(--dsw-alias-label-secondary)}",
			...
			".usg-pv[aria-disabled=true]{color:var(--dsw-alias-label-quaternary);cursor:default}",
			".usg-pv:hover{background:var(--dsw-alias-fill-quaternary)}",
			".usg-pv.usg-in{background:var(--dsw-alias-fill-quinary);border-radius:0}",
			".usg-pv.usg-e{background:var(--dsw-alias-fill-quaternary);border-radius:0}",
			".usg-pop-ft{display:flex;justify-content:space-between;margin-top:8px;border-top:1px solid var(--dsw-alias-separator);padding-top:8px}",
			...
			".usg-list>li:hover{background:var(--dsw-alias-fill-quinary);color:var(--dsw-alias-label-primary)}",
			".usg-list>li[aria-selected=true]{background:var(--dsw-alias-fill-quaternary);color:var(--dsw-alias-label-primary)}",
		].join("\n");
		var cssDone = false;
		function ensureCss() {
		...
		function humanizeError(msg) {
			if (/without inject/.test(msg)) {
				return (
					"服务端接口未挂载：本插件浏览器半已更新到 v3 契约（overview(filter)），而 host 进程还在运行旧版服务端，两端参数名不一致导致 $mount 被网关拒绝——请重启该 profile 的 host（3080 = web profile），再刷新本页。原始错误：" +
					msg
				);
			}
			return msg;
