 ...
 *    全走 --dsw-* 设计令牌。选中配色 = 成熟 range picker 的中性统一语言（用户点名禁 brand 蓝——「AI 最爱用的颜色」）：
 *    月历格 40px、数字 18px/500 flex 真居中（tabular-nums）；端点/pending = 中性深灰实心胶囊（static bluish-700）+ 白字、
 *    区间带与拾取预览同灰色系 color-mix（hover 在带内也加深=鼠标跟手）、今天 = 灰描圈（⚠ 历史坑仍防：alias-brand-primary/
 *    -invert 是墨色 token，浅色主题拿它做填充配对 = 黑底黑字选中即隐形，真机踩实）。
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			/* 月历：40px 格、数字 18px **flex 真居中**（line-height 居中在大字号下会浮上去——用户点名「字占上1/3下面全浪费」）。
			 * 配色 = 成熟 range picker 的中性统一语言（对标 Material/Apple：端点实心胶囊、区间同色系浅带、hover 即时跟随）：
			 * 端点/pending = 中性深灰实底（static bluish-700）+ 白字；区间带（含拾取预览，一套颜色不分两种）
			 * = 同灰 color-mix 22%；hover 在带内也加深一档以示跟随；今天 = 灰圈（不着色、不蓝）。
			 * ⚠ 禁用 alias-brand-primary/-invert/brand-text 做选中填充与字色配对：它们是「墨」（浅色主题双黑=隐形）。 */
			".usg-cal{display:grid;grid-template-columns:repeat(7,40px);gap:2px}",
			".usg-wd{text-align:center;font-size:14px;font-weight:600;color:var(--dsw-alias-label-secondary);line-height:28px}",
			".usg-pv{display:flex;align-items:center;justify-content:center;height:40px;text-align:center;border-radius:10px;cursor:pointer;font-size:18px;font-weight:500;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary)}",
			...
			".usg-pv.usg-today{box-shadow:inset 0 0 0 1.5px var(--dsw-alias-border-l3);font-weight:600}",
			".usg-pv.usg-band,.usg-pv.usg-band:hover{background:color-mix(in srgb, var(--dsw-static-neutral-bluish-700) 18%, transparent);border-radius:10px}",
		...
		function RangePicker(props) {
			...
			function dayCell(d, num) {
				...
				// 拾取中鼠标当前格 = tentative 另一端，也画实心端点胶囊（纯视觉，不占 aria-selected）——
				// 用户点名「鼠标选到哪，哪个就高亮」，不能只在锚点格有反馈。
