...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* ---------- 全部：dock pill + 设置开关（同模块共享，与设置页同 fiber） ---------- */
		/** 开关是浏览器侧纯展示偏好（localStorage，默认开），不进服务端 config/profile。 */
		var FAMILY_KEY = "dsh-plugin-usage-stats.total-pill";
		...
		function FamilyOptRow() {
			...
			var control =
				UI && UI.Switch
					? h(UI.Switch, { checked: en, onChange: setEn, label: "会话底部合计用量开关" })
					: h("input", {
							type: "checkbox",
							role: "switch",
							"aria-checked": en,
							"aria-label": "会话底部合计用量开关",
							checked: en,
							onChange: function (e) {
			...
			return h(
				...
				h(
					...
					h("span", { className: "usg-optLabel" }, "会话底部合计"),
		...
		function FamilyPill(props) {
			...
			// 显示条件：只在开过子代理的主会话显示（sessionCount>1），一看两数不一样就知道哪个是哪个；
			// 关闭/未知/子代理/零用量 → null。可见文案只有数字（宿主 pill 同形，不挂名前缀与命中段）。
			if (!en || !data || !data.known || data.isSubagent || !data.totals || !(data.totals.total > 0) || !(data.sessionCount > 1)) return null;
			var t = data.totals;
