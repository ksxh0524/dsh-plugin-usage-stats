 ...
 *    全走 --dsw-* 设计令牌。月历格 38px/15px 数字（tabular-nums，用户两轮点名「日期字太小」后定的可读性优先尺寸）；
 *    选中端点 = brand 实心药丸（invert 字色）、
 *    区间底 = bg-multi-select、拾取中预览 = interactive-bg-hover-accent（tentative 与已提交视觉分离）、今天 = brand-text。
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			".usg-pre>button{all:unset;cursor:pointer;font-size:13px;line-height:1.5;color:var(--dsw-alias-label-secondary)}",
			...
			".usg-cal-h b{font-size:14px;font-weight:600}",
			...
			/* 月历：38px 见方格 + 15px 等宽数字（旧版星期头用元素选择器误伤数字格、压成 10px 小字，现头/格分类隔离）。
			 * 选中端点 = brand 实心药丸（invert 字色，双主题都高对比）；区间底 = multi-select 填充；
			 * 拾取中的预览 = hover-accent 半透明（与已提交区间区分 tentative）。 */
			".usg-cal{display:grid;grid-template-columns:repeat(7,38px);gap:2px}",
			".usg-wd{text-align:center;font-size:13px;color:var(--dsw-alias-label-tertiary);line-height:26px}",
			".usg-pv{height:38px;line-height:38px;text-align:center;border-radius:9px;cursor:pointer;font-size:15px;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary)}",
			...
			".usg-pv.usg-today{color:var(--dsw-alias-brand-text);font-weight:600}",
			...
			".usg-pv.usg-sel,.usg-pv.usg-sel:hover{background:var(--dsw-alias-brand-primary);color:var(--dsw-alias-brand-primary-invert);font-weight:600}",
			...
			".usg-pop-ft>button{all:unset;box-sizing:border-box;cursor:pointer;font-size:12px;line-height:1.5;padding:2px 6px;margin-left:-6px;border-radius:6px;color:var(--dsw-alias-label-secondary)}",
