...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 宿主 pill 缓存命中段同形：小数命中率 → "90%"（整数不留尾零）。非数回 null（调用方整段不渲染）。 */
		function fmtHit(h) {
			if (typeof h !== "number" || !isFinite(h)) return null;
			return String(Math.round(h * 10000) / 100) + "%";
		}
		...
		function FamilyPill(props) {
			...
			var t = data.totals;
			var hit = fmtHit(data.hitRate);
			var totalText = fmt(t.total) + " tok";
			// 可见文案与宿主 pill 同形（只显示数字，不挂名前缀）；合计身份只进读屏 aria-label。
