 ...
 *    全走 --dsw-* 设计令牌。月历格 32px/13px 数字（tabular-nums）；选中端点 = brand 实心药丸（invert 字色）、
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			".usg-cal-h b{font-size:13px;font-weight:600}",
			".usg-cal-h>button{all:unset;box-sizing:border-box;cursor:pointer;padding:2px 10px;border-radius:6px;font-size:14px;line-height:1.5;color:var(--dsw-alias-label-secondary)}",
			".usg-cal-h>button:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}",
			/* 月历：32px 见方格 + 13px 等宽数字（旧版星期头用元素选择器误伤数字格、压成 10px 小字，现头/格分类隔离）。
			 * 选中端点 = brand 实心药丸（invert 字色，双主题都高对比）；区间底 = multi-select 填充；
			 * 拾取中的预览 = hover-accent 半透明（与已提交区间区分 tentative）。 */
			".usg-cal{display:grid;grid-template-columns:repeat(7,32px);gap:2px}",
			".usg-wd{text-align:center;font-size:12px;color:var(--dsw-alias-label-tertiary);line-height:22px}",
			".usg-pv{height:32px;line-height:32px;text-align:center;border-radius:8px;cursor:pointer;font-size:13px;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary)}",
