...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CONTRIBUTION = {
			...
			descriptors: [descriptor("overview", "filter")],
		};

		/* ---------- 样式（materialize 时注入一次） ----------
		 * token 名以 dsh-client-ui-theme/lib/client.js 的运行时定义表为准（实测核对）：
		 * 边框 border-l1/l2，卡片底 bg-layer-1/2，浮层 specific-menu + shadow-lv3，
		 * 按钮 button-tool-bar-*，选中态 button-ghost-active-*，交互 interactive-bg-*，
		 * 文字 label-primary/secondary/tertiary/dimmed，错误 state-error-primary。
		 * 教训：曾用 --dsw-alias-separator/fill-quinary 等（MUI 风格名，宿主根本没定义），
		 * 真页上边框/浮层底色整片解析失效——harness 自造变量还掩盖了它。 */
		var CSS = [
			".usg-root{display:flex;flex-direction:column;gap:14px;padding:16px 18px;overflow:auto;height:100%;box-sizing:border-box;font-size:13px;color:var(--dsw-alias-label-primary);max-width:980px}",
			...
			".usg-kpi{border:1px solid var(--dsw-alias-border-l1);border-radius:8px;padding:8px 10px;display:flex;flex-direction:column;gap:2px;background:var(--dsw-alias-bg-layer-1)}",
			...
			".usg-table th{text-align:right;font-weight:500;color:var(--dsw-alias-label-tertiary);padding:4px 6px;border-bottom:1px solid var(--dsw-alias-border-l1);white-space:nowrap}",
			".usg-table td{text-align:right;padding:4px 6px;border-bottom:1px solid var(--dsw-alias-border-l1);white-space:nowrap;font-variant-numeric:tabular-nums}",
			".usg-table th:first-child,.usg-table td:first-child{text-align:left}",
			".usg-muted{color:var(--dsw-alias-label-tertiary)}",
			".usg-err{color:var(--dsw-alias-state-error-primary);padding:8px;border:1px solid currentColor;border-radius:8px}",
			".usg-btn{all:unset;box-sizing:border-box;cursor:pointer;padding:4px 12px;border:1px solid var(--dsw-alias-border-l2);border-radius:6px;font-size:12px;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-button-tool-bar-fill)}",
			".usg-btn:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-button-tool-bar-hover)}",
			".usg-btn[aria-pressed=true]{background:var(--dsw-alias-button-ghost-active-fill);border-color:var(--dsw-alias-button-ghost-active-border);color:var(--dsw-alias-label-primary)}",
			".usg-title{font-size:12px;font-weight:600;color:var(--dsw-alias-label-secondary);margin-bottom:4px}",
			/* 浮层（RangePicker / DimSelect 共用）：透明 mask 兜外点关闭，panel 用宿主菜单底色+阴影 */
			".usg-anchor{position:relative}",
			".usg-mask{position:fixed;inset:0;z-index:60}",
			".usg-pop{position:absolute;top:calc(100% + 4px);left:0;z-index:61;border:1px solid var(--dsw-alias-border-l2);border-radius:10px;background:var(--dsw-specific-menu);box-shadow:var(--dsw-shadow-lv3);padding:10px;font-size:12px;min-width:230px}",
			".usg-pre{display:flex;gap:4px;flex-wrap:wrap;margin-bottom:8px}",
			".usg-pre>button{all:unset;cursor:pointer;padding:2px 8px;border:1px solid var(--dsw-alias-border-l2);border-radius:999px;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-button-tool-bar-fill)}",
			".usg-pre>button:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-button-tool-bar-hover)}",
			".usg-cal-h{display:flex;align-items:center;justify-content:space-between;margin-bottom:4px}",
			".usg-cal-h b{font-size:12px}",
			".usg-cal-h>button{all:unset;cursor:pointer;padding:0 8px;border-radius:6px;color:var(--dsw-alias-label-secondary)}",
			...
			".usg-pop-ft{display:flex;justify-content:space-between;margin-top:8px;border-top:1px solid var(--dsw-alias-border-l1);padding-top:8px}",
			...
			var load = useCallback(
				async function () {
					...
					try {
						...
						var r = await ctx.remote.usageStats.overview(f);
			...
			return h(
				...
				data
					? h(
							...
							msgs ? kpi("消息数", "用 " + fmt(msgs.user) + " · 答 " + fmt(msgs.assistant) + " · 工 " + fmt(msgs.toolCalls)) : null,
				...
				data
					? h(
							...
							" · 价目编辑在「设置 → 插件」的用量卡（settings.yaml → usage-stats.prices） · 数据截至 " + new Date(data.generatedAt).toLocaleTimeString(),
						)
					: null,
			);
		}
		...
		function humanizeError(msg) {
			if (/without inject/.test(msg)) {
				return (
					"服务端接口未挂载：本插件浏览器半已更新到 v3 契约（overview(filter)），而 host 进程还在运行旧版服务端，两端参数名不一致导致 $mount 被网关拒绝——重启你当前 profile 的 host 后刷新本页即可（服务端改动必须重启生效）。原始错误：" +
					msg
				);
			}
			return msg;
