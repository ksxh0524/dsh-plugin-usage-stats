...
- **Report**: sessions scanned, messages (user / assistant / tool calls), total tokens, uncached input / output / cache read (with hit rate) / cache write, total cost, then per-model and per-day tables, and a read-only price summary at the bottom (editing goes through settings.yaml, see below). Deliberately lightweight — no price-editing form.
