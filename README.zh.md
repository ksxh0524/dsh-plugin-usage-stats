...
- 合计 pill 自动刷新：子代理开/关、本会话落步、3s 服务端轮询三路触发——开出子代理后 pill 自动出现，用量涨了数字自动跟上，不用手动刷新页面。轮询走增量扫描（未变文件只做 stat），页签切后台时暂停。

## 已知边界

- 增量解码依赖帧 walker 识别「完整帧」：文件尾半帧留待下次补齐；legacy/字典帧退化为整文件重扫（仍正确，只是慢）。
- 设置页：打开页面、手动刷新、改筛选时拉取；无服务端推送，也不自动轮询（合计 pill 的自动刷新见上节）。
